...
def export_preoperacional_to_excel(request):
    ...
    preoperacional_diarios = PreoperacionalDiario.objects.filter(fecha__year=anio, fecha__month=mes)

    headers = [
        'Fecha', 'Vehículo', 'Responsable', 'Kilometraje', 'Nivel de Combustible',
        'Nivel de Aceite', 'Nivel de Refrigerante', 'Nivel de Hidráulico',
        'Nivel de Líquido de Frenos', 'Poleas', 'Correas', 'Mangueras', 'Acoples',
        'Tanques', 'Radiador', 'Terminales', 'Bujes', 'Rótulas', 'Ejes', 'Cruceta',
        'Puertas', 'Chapas', 'Manijas', 'Elevavidrios', 'Lunas', 'Espejos',
        'Vidrio Panorámico', 'Asiento', 'Apoyacabezas', 'Cinturón', 'Aire',
        'Caja de Cambios', 'Dirección', 'Batería', 'Luces Altas', 'Luces Medias',
        'Luces Direccionales', 'Cocuyos', 'Luz de Placa', 'Luz Interna', 'Pito',
        'Alarma de Retroceso', 'Arranque', 'Alternador', 'Rines', 'Tuercas',
        'Esparragos', 'Freno de Servicio', 'Freno de Seguridad', 'Llanta de Repuesto',
        'Llantas', 'Suspensión', 'Capó', 'Persiana', 'Bumper Delantero', 'Parabrisas',
        'Guardafango', 'Stop', 'Bumper Trasero', 'Vidrio Panorámico Trasero',
        'Placa Delantera', 'Placa Trasera', 'Aseo Externo', 'Aseo Interno',
        'Kit de Carreteras', 'Kit de Herramientas', 'Kit de Botiquín',
        'Chaleco Reflectivo', 'Aprobado', 'Observaciones'
    ]
    ...
    for preop in preoperacional_diarios:
        fila = [
            ...
            (f"{preop.reporter.first_name} {preop.reporter.last_name}"
                if preop.reporter else preop.nombre_no_registrado),
            preop.kilometraje,
    ...
    return pro_export_to_excel(model=PreoperacionalDiario, headers=headers, data=data, filename=filename, sheet_name=sheet_name, table_title=table_title)
...
def export_salidas_to_excel(request):
    ...
    preoperacionales = Preoperacional.objects.filter(
        fecha__month=mes,
        fecha__year=anio
    )
